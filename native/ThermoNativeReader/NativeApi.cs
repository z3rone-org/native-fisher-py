...
namespace ThermoNativeReader
{
    public static class NativeApi
    {
        ...
        public unsafe struct CentroidParams
        {
            ...
            public int* Charges;
            public double* NoiseRes;
            public int MaxLength;
        }

        [StructLayout(LayoutKind.Sequential)]
        public unsafe struct ChromatogramParams
        {
            public int TraceType;
            public byte* Filter;
            public double* MassRangesStart;
            public double* MassRangesEnd;
        ...
        public static unsafe void SelectInstrument(long arg0, int arg1, int arg2)
        {
            RunOnWorkerSync(() => {
                if (_openFiles.TryGetValue(arg0, out var f)) {
                    f.SelectInstrument((Device)arg1, arg2);
                }
            });
        }
        ...
        public static unsafe int GetNumScans(long arg0)
        {
            return RunOnWorker(() => {
                ...
                try {
                    int count = f.RunHeaderEx.SpectraCount;
                    if (count <= 0) count = f.RunHeader.LastSpectrum - f.RunHeader.FirstSpectrum + 1;
                    return count;
                } catch { 
                    try { return f.RunHeader.LastSpectrum - f.RunHeader.FirstSpectrum + 1; } catch { return 0; }
                }
            });
        }
        ...
        public static unsafe int IsCentroid(long arg0, int arg1)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try { 
                    var scanStatistics = f.GetScanStatsForScanNumber(arg1);
        ...
        public static unsafe int GetCentroidStreamFull(long arg0, int arg1, CentroidParams* p)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try {
        ...
        public static unsafe int GetScanEventString(long arg0, int arg1, byte* arg2, int arg3)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try {
                    string s = f.GetFilterForScanNumber(arg1).ToString();
                    return CopyString(s, arg2, arg3);
                } catch { return 0; }
            });
        }
        ...
        public static unsafe int GetChromatogram(long arg0, ChromatogramParams* p)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try { 
                    string filter = Marshal.PtrToStringAnsi((IntPtr)p->Filter) ?? "";
                    var settings = new ChromatogramTraceSettings((TraceType)p->TraceType) { Filter = filter };
                    
                    if (p->MassRangeCount > 0)
                    {
                        settings.MassRangeCount = p->MassRangeCount;
                        for (int i = 0; i < p->MassRangeCount; i++)
                        {
                            settings.SetMassRange(i, new Range(p->MassRangesStart[i], p->MassRangesEnd[i]));
                        }
                    }

                    var data = f.GetChromatogramData(new[] { settings }, p->StartScan, p->EndScan);
                    ...
                    int count = Math.Min(data.PositionsArray[0].Length, p->MaxLength);
                    for (int i = 0; i < count; i++)
                    {
                        p->Times[i] = data.PositionsArray[0][i];
                        p->Intensities[i] = data.IntensitiesArray[0][i];
                    }
                    return count;
                } catch { return 0; }
            });
        ...
        public static unsafe int GetFilters(long arg0, IntPtr* arg1, int arg2)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try { 
                    var filterList = f.GetFilters().ToArray();
                    int count = Math.Min(filterList.Length, arg2);
                    for (int i = 0; i < count; i++)
                    {
                        arg1[i] = Marshal.StringToHGlobalAnsi(filterList[i].ToString());
                    }
        ...
        public static unsafe int GetScanFilterMetaFilters(long arg0, int arg1, IntPtr* arg2, int arg3)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                try {
                    var filter = f.GetFilterForScanNumber(arg1);
                    if (filter == null) return 0;
                    
                    // Reflection based access to MetaFilters (if not in public interface)
                    ...
                    int count = Math.Min(metaList.Count, arg3);
                    for (int i = 0; i < count; i++)
                    {
                        arg2[i] = Marshal.StringToHGlobalAnsi(metaList[i]);
                    }
        ...
        public static unsafe double GetScanEventPrecursorMass(long arg0, int arg1, int arg2)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return -1.0;
                try { 
                    var ev = f.GetScanEventForScanNumber(arg1);
                    if ((int)ev.MSOrder > 1 && arg2 < ev.MassCount) return ev.GetMass(arg2);
                    return 0.0;
                } catch { return -1.0; }
            });
        }
        ...
        public static unsafe int GetScanEventActivationType(long arg0, int arg1, int arg2)
        {
            return RunOnWorker(() => {
                ...
                try { 
                    var ev = f.GetScanEventForScanNumber(arg1);
                    if ((int)ev.MSOrder > 1 && arg2 < ev.MassCount) return (int)ev.GetReaction(arg2).ActivationType;
                    return -1; // -1 for "None" or "Not Applicable"
                } catch { return 0; }
            });
        }
        
        [UnmanagedCallersOnly(EntryPoint = "get_scan_event_activation_type_name")]
        public static unsafe int GetScanEventActivationTypeName(long arg0, int arg1, int arg2, byte* arg3, int arg4)
        {
            return RunOnWorker(() => {
                if (!_openFiles.TryGetValue(arg0, out var f)) return 0;
                if (arg3 == null || arg4 <= 0) return 0;
                try { 
                    var ev = f.GetScanEventForScanNumber(arg1);
                    string name = "None";
                    if ((int)ev.MSOrder > 1 && arg2 < ev.MassCount) name = ev.GetReaction(arg2).ActivationType.ToString();
                    var bytes = System.Text.Encoding.UTF8.GetBytes(name);
                    int len = Math.Min(bytes.Length, arg4 - 1);
                    for (int i = 0; i < len; i++) arg3[i] = bytes[i];
                    arg3[len] = 0;
                    return len;
                } catch { return 0; }
            });
        ...
        public static unsafe double GetScanEventCollisionEnergy(long arg0, int arg1, int arg2)
        {
            return RunOnWorker(() => {
                ...
                try { 
                    var ev = f.GetScanEventForScanNumber(arg1);
                    if ((int)ev.MSOrder > 1 && arg2 < ev.MassCount) return ev.GetEnergy(arg2);
                    return 0.0;
                } catch { return -1.0; }
            });
        }
