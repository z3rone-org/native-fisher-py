...
#[repr(C)]
struct ChromatogramParams {
    trace_type: i32,
    filter: *const c_char,
    mass_ranges_start: *const f64,
    mass_ranges_end: *const f64,
    mass_range_count: i32,
    start_scan: i32,
    end_scan: i32,
    times: *mut f64,
    intensities: *mut f64,
...
fn get_centroid_stream(handle: i64, scan_number: i32, max_length: i32) -> PyResult<(Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>, Vec<i32>, f64, f64)> {
    ...
    let mut params = CentroidParams {
        masses: masses.as_mut_ptr(),
        intensities: intensities.as_mut_ptr(),
        baselines: baselines.as_mut_ptr(),
        noises: noises.as_mut_ptr(),
        charges: charges.as_mut_ptr(),
        noise_res: noise_res.as_mut_ptr(),
        max_length,
    };

    unsafe {
        let func: Symbol<unsafe extern "C" fn(i64, i32, *mut CentroidParams) -> i32> = 
...
fn get_chromatogram(handle: i64, trace_type: i32, filter: String, mass_ranges_start: Vec<f64>, mass_ranges_end: Vec<f64>, start_scan: i32, end_scan: i32, max_length: i32) -> PyResult<(Vec<f64>, Vec<f64>)> {
    ...
    let mut params = ChromatogramParams {
        trace_type: trace_type,
        filter: c_filter.as_ptr(),
        mass_ranges_start: mass_ranges_start.as_ptr(),
        mass_ranges_end: mass_ranges_end.as_ptr(),
        mass_range_count: mass_ranges_start.len() as i32,
    ...
    unsafe {
        let func: Symbol<unsafe extern "C" fn(i64, *mut ChromatogramParams) -> i32> = lib.get(b"get_chromatogram")
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("get function get_chromatogram: {}", e)))?;
        let count_res = func(handle, &mut params);
        if count_res < 0 {
