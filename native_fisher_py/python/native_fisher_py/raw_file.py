...
class RawFile(object):
    ...
    def get_trailer_extra_information(self, scan_number): 
        ...
        values = get_trailer_extra_values(self._handle, scan_number)
        return LogEntry(values, labels)
    ...
    def get_trailer_extra_values(self, scan_number): 
    ...
    def get_status_log_values(self, scan_number):
        from .data.classes import LogEntry
        labels = [h.label for h in self.get_status_log_header_information()]
        values = get_status_log_values(self._handle, scan_number)
        return LogEntry(values, labels)
    ...
    def select_instrument(self, device_type: int, instrument_index: int = 1):
        from .native_fisher_py_backend import select_instrument
        select_instrument(self._handle, device_type, instrument_index)
